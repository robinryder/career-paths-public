...
#### 6. Analysis with only Linkedin ####

#### 6.1 Inference for study 1 ####

# With only link 
dataTot = split(rbindlist(readRDS("data/clean/dataTot.rds"))[profile_present==1,], by ="ident")
...
res_link_only = inference_mod(formulas = formulas4, groups = ("ena"), dataTot = dataTot, model_index = 1,
                           N = 5000, size_simu = 1, iter_metropolis = 30, obsburn = 5000,
                           prop_of_observed = 1, contr_simulated = 1, remove_early = F, ena_pop = T)


saveRDS(res_link_only, "data/revise_resubmit/mod4_linkedin_only.rds")

#### 6.2 Study 2 ####

dataTot = split(rbindlist(readRDS("data/clean/dataTot.rds"))[profile_present==1,], by ="ident")

formulas = list("gamma0"= traj_lead ~ -1+group,
     "gamma1"= I(1-traj_lead) ~ -1+group,
     "lambda"= trace_obs ~ -1+group
     +group:(lagged1+lagged2+sqrt(lagged3))+as.double(lagged1==0):bs(age_max,df=5,degree=2,Boundary.knots=c(0,1),intercept=T))

groups = c("cprefet",  "ccomptes", "iga"   ,   "ce"    ,   "cdiplo",   "igf"    ,  "dgfip"  ,  "insee" ,   "igas"    , "dgtresor")

res = inference_mod(formulas = formulas, groups = groups, dataTot = dataTot, model_index = 2,
              N = 5000, size_simu = 1, iter_metropolis = 30, obsburn = 5000, 
              prop_of_observed = 1, contr_simulated = 1, remove_early = FALSE, ena_pop = FALSE)
saveRDS(res, "data/revise_resubmit/groups_config_1_alt.rds")
...
res_link_only = readRDS("data/revise_resubmit/mod4_linkedin_only.rds")
res_normal = readRDS("data/results/ena_config_4.rds")

span = 300:5000
library(ggplot2)

# dataframe avec parameter, 

res = list()
res[[length(res)+1]] = data.table(x = res_normal$params$gamma0[6,span], par = "Gamma 0", subpar = "period", type = "Baseline", study = 1)
res[[length(res)+1]] = data.table(x = res_normal$params$gamma1[6,span], par = "Gamma 1", subpar = "period", type = "Baseline", study = 1)
res[[length(res)+1]] = data.table(x = res_link_only$params$gamma0[6,span], par = "Gamma 0", subpar = "period", type = "Only profiles", study = 1)
res[[length(res)+1]] = data.table(x = res_link_only$params$gamma1[6,span], par = "Gamma 1", subpar = "period", type = "Only profiles", study = 1)

res2 = rbindlist(res)

ggplot(res2) + geom_histogram(aes(x =x), fill = "white", col = "black") + facet_grid(cols = vars(par), rows = vars(type), scales = "free") + theme_bw() + ylab("Posterior draws") + xlab("Period coefficient (raw)")

ggsave("documents/figures/RR_only_profiles_study1.pdf", units = "cm", width = 10, height = 6)

## Study 2

# Analysis

res_new = readRDS("data/revise_resubmit/groups_config_1_alt.rds")
res_original = readRDS("data/results/groups_config_1.rds")

varnames = rownames(res_original$params$gamma1)
span = 300:5000
res = list()
res[[1]] = data.table()
for (i in 1:10){
  res[[length(res)+1]] = data.table(x = res_original$params$gamma0[i,span], par = "gamma0", subpar = varnames[i], type = "Baseline", study = 2)
  res[[length(res)+1]] = data.table(x = res_original$params$gamma1[i,span], par = "gamma1", subpar = varnames[i], type = "Baseline", study = 2)
  res[[length(res)+1]] = data.table(x = res_new$params$gamma0[i,span], par = "gamma0", subpar = varnames[i], type = "Only profiles", study = 2)
  res[[length(res)+1]] = data.table(x = res_new$params$gamma1[i,span], par = "gamma1", subpar = varnames[i], type = "Only profiles", study = 2)
}


res2 = rbindlist(res)

res2$subpar = substr(res2$subpar, 6, nchar(res2$subpar))
res2$prob = plogis(res2$x)

stats_df0 <- res2[par == "gamma0", ] %>%
  group_by(subpar, type) %>%
  summarise(
    p10 = quantile(prob, 0.10),
    mean = mean(prob),
    p90 = quantile(prob, 0.90),
    .groups = "drop")

stats_df1 <- res2[par == "gamma1", ] %>%
  group_by(subpar, type) %>%
  summarise(
    p10 = quantile(prob, 0.10),
    mean = mean(prob),
...
# Plot
ggplot(res2[par == "gamma0", ]) +
  geom_histogram(aes(x = prob), bins = 30, fill = "white", color = "black") +
  geom_vline(data = stats_df0, aes(xintercept = mean), color = "darkgrey", linetype = "solid") +
  geom_vline(data = stats_df0, aes(xintercept = p10), color = "darkgrey", linetype = "longdash") +
  geom_vline(data = stats_df0, aes(xintercept = p90), color = "darkgrey", linetype = "longdash") +
  facet_grid(cols = vars(subpar), rows = vars(type), scales = "free_x") + 
  scale_y_continuous("Posterior draws") + theme_bw() +
  xlab(expression("Posterior probability ("*gamma[0]*")")) +
  theme(axis.text.x = element_text(size = 4))

ggsave("documents/figures/RR_only_profiles_study2_gamma0.pdf", units = "cm", width = 16, height = 8)

ggplot(res2[par == "gamma1", ]) +
  geom_histogram(aes(x = prob), bins = 30, fill = "white", color = "black") +
  geom_vline(data = stats_df1, aes(xintercept = mean), color = "darkgrey", linetype = "solid") +
  geom_vline(data = stats_df1, aes(xintercept = p10), color = "darkgrey", linetype = "longdash") +
  geom_vline(data = stats_df1, aes(xintercept = p90), color = "darkgrey", linetype = "longdash") +
  facet_grid(cols = vars(subpar), rows = vars(type), scales = "free_x") + 
  scale_y_continuous("Posterior draws") + theme_bw() +
  xlab("Predicted probability (Gamma 1)") +
  theme(axis.text.x = element_text(size = 4))

ggsave("documents/figures/RR_only_profiles_study2_gamma1.pdf", units = "cm", width = 16, height = 8)


#### 7. Analysis without Linkedin ####

dataTot = split(rbindlist(readRDS("data/clean/dataTot.rds")), by ="ident")

dt = rbindlist(readRDS("data/clean/dataTot.rds"))
dt$traj_obs = 98
dataTot = split(dt, by = "ident")
...
res = inference_mod(formulas = formulas4, groups = ("ena"), dataTot = dataTot, model_index = 1,
              N = 100, size_simu = 1, iter_metropolis = 30, obsburn = 20,
              prop_of_observed = 1, contr_simulated = 1, remove_early = F, ena_pop = T)

saveRDS(res, "data/revise_resubmit/mod4_without_linkedin.rds")


res = readRDS("data/revise_resubmit/mod4_without_linkedin.rds")
rowMeans(res$params$lambda)
rowMeans(res$params$gamma0)
rowMeans(res$params$gamma1)
