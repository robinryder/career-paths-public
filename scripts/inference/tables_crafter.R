...
configs = expand.grid("model_index"=1:5,"linkedin_correct"=1,
                      "N"=1000, "burnin"=900, "size_simu"=1,"iter_metropolis" = 30,
...
for (i in 1:nrow(configs)){
  ...
  dt[[i]]$obs = fread(paste0("data/results/ena_config_w_",i,".csv"))
}
...
y0 = bs((0:100)/100, df=5, intercept=F, degree=2,Boundary.knots = c(0,1)) %*% dt[[3]]$params[["gamma0"]][2:6,span]
for (i in 1:101) y0[i,] = y0[i,]+dt[[3]]$params[["gamma0"]][1,span]

y1 = bs((0:100)/100, df=5, intercept=F, degree=2,Boundary.knots = c(0,1)) %*% dt[[3]]$params[["gamma1"]][2:6,span]
...
ggplot(res) + geom_line(aes(x = time, y = plogis(value), color = type, linetype = variable)) + 
  scale_x_continuous("Years since admission") + 
  scale_y_continuous("Averaged proability in six months", limits = c(0,0.1)) + 
  scale_color_manual("Coefficient", values=c("black","darkgrey")) + 
  scale_linetype_manual("Quantile", values =c("q50"="solid","q05"="dashed","q95"="dashed")) +
  theme_bw() 


ggplot(res[plogis(value)<=0.15]) + geom_line(aes(x = time, y = plogis(value), linetype = variable)) + 
  scale_x_continuous("Years since admission") + 
  scale_y_continuous("Averaged proability in six months") + 
  scale_linetype_manual("Quantile", values =c("q50"="solid","q05"="dashed","q95"="dashed")) +
  theme_bw() + facet_grid(type~.,scales="free")

ggsave(units = "cm", width = 11, height = 7, filename = "documents/figures/time_prob.png")
...
table2 = ggplot(res) + 
  geom_histogram(aes(x = value),col = "darkgrey",fill="darkgrey") + 
...
ggsave(table2, 
       units = "cm", 
       width = 12, 
       height = 9, filename = "documents/figures/figure_models45.png",scale = 1)

#### Figure 4: proportion 10 years after ####
...
ggsave(units = "cm", width = 9, height = 9, filename = "documents/figures/model1_coeffs.png",scale = 1)
...
r2 = rbindlist(r2a)
r2$org = factor(r2$org, levels=groups)
...
plot2 = ggplot(r2,aes(p)) + 
  facet_wrap(.~org, ncol = 3) + theme_bw() +
  geom_histogram(bins = 30, col = "darkgrey", fill = "darkgrey") +
  geom_vline(xintercept = 1, col = "black") +
  scale_x_continuous("Women vs. men",limits = c(0,2)) + scale_y_continuous("") + 
  theme(text = element_text(size = 9))     


plot3 = ggplot(r3,aes(p)) + 
  facet_wrap(.~org, ncol = 3) + theme_bw() +
  geom_histogram(bins = 30, col = "darkgrey", fill = "darkgrey") +
  geom_vline(xintercept = 1, col = "black") +
  scale_x_continuous("ENA vs. non ENA") + scale_y_continuous("") + 
  theme(text = element_text(size = 9))    

g = grid.arrange(plot2, plot3, ncol = 2)



ggsave(g, units = "cm", width = 15, height = 11, filename = "documents/figures/model2_coeffs.png",scale = 1)
